...
using DemoX.Models;
using DemoX.Services;
using DemoX.Entities;
using DemoX.Filter;
using DemoX.Authorization;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.JsonPatch;

namespace DemoX.Controllers
{
    /// <summary>
    /// Controller responsible for managing medicationcomposition related operations.
    /// </summary>
    /// <remarks>
    ...
    {
        private readonly IMedicationCompositionService _medicationCompositionService;

        /// <summary>
        /// Initializes a new instance of the MedicationCompositionController class with the specified context.
        /// </summary>
        /// <param name="imedicationcompositionservice">The imedicationcompositionservice to be used by the controller.</param>
        public MedicationCompositionController(IMedicationCompositionService imedicationcompositionservice)
        {
            _medicationCompositionService = imedicationcompositionservice;
        }
        ...
        {
            var id = _medicationCompositionService.Create(model);
            return Ok(new { id });
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var result = _medicationCompositionService.Get(filterCriteria, searchTerm, pageNumber, pageSize, sortField, sortOrder);
            return Ok(result);
        }
        ...
        {
            var result = _medicationCompositionService.GetById(id);
            return Ok(result);
        }
        ...
        {
            var status = _medicationCompositionService.Delete(id);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] MedicationComposition updatedEntity)
        {
            ...
            var status = _medicationCompositionService.Update(id, updatedEntity);
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<MedicationComposition> updatedEntity)
        {
            ...
            var status = _medicationCompositionService.Patch(id, updatedEntity);
            return Ok(new { status });
        }
    }
}